...
func (c *Client) do(ctx context.Context, method, path string, query url.Values, body []byte, senderAuth bool) (status int, respBody []byte, hdr http.Header, err error) {
	u, err := url.Parse(c.baseURL + path)
	if err != nil {
		return 0, nil, nil, fmt.Errorf("parse url: %w", err)
	}
	if query != nil {
	...
	if senderAuth {
		if c.apiKey == "" {
			return 0, nil, nil, fmt.Errorf("PAYCREST_API_KEY is required for sender routes")
		}
	...
	if int64(len(data)) > c.maxBody {
		return res.StatusCode, nil, res.Header, fmt.Errorf("response exceeds PAYCREST_MAX_RESPONSE_BYTES (%d)", c.maxBody)
	}
...
func (c *Client) Get(ctx context.Context, path string, query url.Values, senderAuth bool) (status int, body []byte, err error) {
	st, b, _, err := c.do(ctx, http.MethodGet, path, query, nil, senderAuth)
	return st, b, err
}

// GetForPoll performs GET and returns response headers (e.g. Retry-After on 429).
func (c *Client) GetForPoll(ctx context.Context, path string, query url.Values, senderAuth bool) (status int, body []byte, hdr http.Header, err error) {
...
func (c *Client) PostJSON(ctx context.Context, path string, payload []byte, senderAuth bool) (status int, body []byte, err error) {
	st, b, _, err := c.do(ctx, http.MethodPost, path, nil, payload, senderAuth)
	return st, b, err
}
